...
local OrionLib = {
	...
	Themes = {
		...
		WinterBlue = {
			...
			Second = Color3.fromRGB(30, 55, 80),
			Stroke = Color3.fromRGB(60, 120, 200),
			Divider = Color3.fromRGB(40, 80, 130),
			Text = Color3.fromRGB(220, 240, 255),
			TextDark = Color3.fromRGB(150, 200, 240)
		}
	},
	SelectedTheme = "WinterBlue",
	Folder = nil,
	SaveCfg = false
}

-- Safe HTTP request for icons (with fallback)
local Icons = {}
local Success, Response = pcall(function()
	local request = syn and syn.request or request or http_request
	if request then
		local result = request({
			Url = "https://raw.githubusercontent.com/evoincorp/lucideblox/master/src/modules/util/icons.json",
			Method = "GET"
		})
		if result and result.Body then
			Icons = HttpService:JSONDecode(result.Body).icons
		end
	end
end)

if not Success or not next(Icons) then
...
Orion.Parent = GetParent()

-- Clean up existing Orion GUIs
local function CleanOrion()
...
local function SetChildren(Element, Children)
	for _, Child in pairs(Children) do
		pcall(function() Child.Parent = Element end)
	end
	return Element
end
		...
		if OrionLib.Flags[a] then
			task.spawn(function() 
				if OrionLib.Flags[a].Type == "Colorpicker" then
					pcall(function() OrionLib.Flags[a]:Set(UnpackColor(b)) end)
				else
					pcall(function() OrionLib.Flags[a]:Set(b) end)
				end    
			end)
		end
	end
...
-- Create Elements (simplified for compatibility)
...
CreateElement("Image", function(ImageID)
	...
	if icon then
		pcall(function() ImageNew.Image = icon end)
	end	
...
function OrionLib:MakeNotification(NotificationConfig)
	task.spawn(function()
		...
		local NotificationFrame = SetChildren(SetProps(MakeElement("RoundFrame", OrionLib.Themes[OrionLib.SelectedTheme].Main, 0, 10), {
			...
			MakeElement("Stroke", OrionLib.Themes[OrionLib.SelectedTheme].Stroke, 1.2),
...
function OrionLib:MakeWindow(WindowConfig)
	...
	local WindowStuff = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", OrionLib.Themes[OrionLib.SelectedTheme].Main, 0, 10), {
		...
		SetChildren(SetProps(MakeElement("TFrame"), {
			...
			AddThemeObject(SetProps(MakeElement("Label", "@k0nvlct", 12), {
	...
	function TabFunction:MakeTab(TabConfig)
		...
		if icon then
			pcall(function() TabFrame.Ico.Image = icon end)
		end	
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddLabel(Text)
				...
				function LabelFunction:Set(ToChange)
					local content = LabelFrame:FindFirstChild("Content")
					if content then
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				ToggleConfig.Color = ToggleConfig.Color or Color3.fromRGB(60, 160, 255)
			...
			function ElementFunction:AddSlider(SliderConfig)
				...
				SliderConfig.Color = SliderConfig.Color or Color3.fromRGB(60, 160, 255)
			...
			function ElementFunction:AddDropdown(DropdownConfig)
				...
				local function AddOptions(Options)
					for _, Option in pairs(Options) do
						local OptionBtn = AddThemeObject(SetProps(SetChildren(MakeElement("Button", OrionLib.Themes[OrionLib.SelectedTheme].Divider), {
			...
			function ElementFunction:AddBind(BindConfig)
				...
				function Bind:Set(Key)
					...
					if type(Bind.Value) == "userdata" then
			...
			function ElementFunction:AddColorpicker(ColorpickerConfig)
				...
				local ColorpickerBox = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", OrionLib.Themes[OrionLib.SelectedTheme].Main, 0, 4), {
					Size = UDim2.new(0, 24, 0, 24),
					Position = UDim2.new(1, -12, 0.5, 0),
					AnchorPoint = Vector2.new(1, 0.5)
				}), {
